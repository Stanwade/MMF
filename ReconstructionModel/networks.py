...
class PositionalEncoding2D(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.cat([x, sin_x, cos_x, sin_y, cos_y], dim=1)
...
class FullyConnectedNetwork(nn.Module):
    def __init__(self,
        ...
        if with_pe:
            self.pe = PositionalEncoding2D(in_img_shape[0], in_img_shape[1], in_img_shape[2])
            pe_out_channels = self.pe.calculate_channel_dim()
            self.blocks.append(self.pe)
            self.blocks.append(nn.Linear(pe_out_channels, mid_lengths[-1]))
            self.blocks.append(self.act)
